...
Make a file sr_xissues.R that adds xissue suppport to our shiny app. The idea is that I want to systematically describe and store common failure modes that are expected issues (xissues).

1. Make a new pane xissue

2. It shall have a text area field "xissue_yaml_text" where I can write a yaml text to add as an xissue by pressing a button below

3. In xissues.R make a function:
...
4. Below have a table with current xissues. When clicking on an xissue create yaml text in the text field but don't save anything, it is like a template I can modify for a new xissue. 

5. When I click in the issues pane on a project, write an xissue_tpl_last.yaml file into the output dir that is a yaml template for the first issue for the first PID of that error type. E.g. you can extract cmd, failure_cat, etc. Other fields shall be empty quotes, like descr or the search patterns. Fill also "xissue_yaml_text" with it but if xissue_yaml_text also contains a text save it before in a file output_dir/xissue_tpl_old.yaml as a backup

6. When starting the app see if xissue_tpl_last.yaml exists, if yes prefill xissue_yaml_text with it. Otherwise empty.

7. If you have more questions ask, don't just implement the things differently.

Provide the code and explain your changes.
