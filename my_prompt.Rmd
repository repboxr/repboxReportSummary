...
Currently, we have expected issues xissues that we can describe. Can we also create similar lists for articles that we want to ignore (put on bottom of the list). We only want to store to fields: artid, descr

descr is an optional short description why we want to ignore that article (e.g. "corrupted raw data" etc). The list shall be stored in the same folder as xissues.


Add a button to ignore the article when pressing on an issue of the article. As with xissues also allow to delete the entry for an artid.

Explain your code modifications.
