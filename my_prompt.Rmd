...
I have added a field xid to provide an id for an xisssue. Please implement:

- If an issue with the given xid already exists, replace it instead of adding a new issue with same issue. Also force a non-empty xid. 

- Add functionality with faster helper functions in xissues.R or sr_xissues.R to check whether an actual regcheck issue matches a specified xissue. Rules:
  - If no pattern is specified just check whether failure mode fits and cmd fits
  - If no cmd is specified check for failure mode and pattern
  - Note that we may have multiple commands (comma separated list)
  - Convert a fixed pattern to regex such that spaces will be replaced by general word boundaries.
  - Use stringi
  
- If an issue matches an existing xissue show it in the issue list under the xid and put issues that match xid at the bottom of the table. Idea: we are mainly interested in dealing with new issues that have not already been characterized as an xissue.

- Correspondingly if clicking on an issue that has an existing xissue, pre-fill the text area for the xissues with exactly that issue instead of generating a generic template.

Does this all make sense? If not ask questions.

Otherwise directly implement and explain beforehand what you have done.
