...
Please implement a new feature. When clicking on an article in the issue tab and details are shown please add another button on the right with label "Set low priority".

In the same output folder where xissue files are stored generate a text file low_priority_issues_artid.txt. It shall contain the artid of articles whose issues are marked as low priority. Issues of a low priority article shall be sorted to the bottom of the issue list. Idea is that these are not xissues but smaller issues, I won't handle now. It is ok to mark complete articles even if there are different issue classes, practically I will always check complete articles. 

If a low priority article is opened show a button "Set normal priority" that allows to remove it from the low priority list.
